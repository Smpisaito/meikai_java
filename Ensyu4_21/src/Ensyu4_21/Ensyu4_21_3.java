...
public class Ensyu4_21_3 {
	...
	/*
	 * 関数名：main
	 * 概要：入力した段数の長さの2辺になる直角三角形を表示する。
	 *       演習4-21の問いにおける、直角三角形の直角が「右上側」の場合をつくる。
	 * 引数：なし
	 * 戻り値：なし
	 * 作成者：S.Saito
	 * 作成日：2023.04.12
...
// 入力をキーボードに指定する。main内で入力操作を行うコードより前に置く。
		...
		for ( int firstNumber = 1 ; firstNumber <= integerNumber; firstNumber++ )  {
			// 「空白」を表示するループ。繰り返す回数を0回から縦軸の段数が変わるごとに1回ずつ増やしていく。
			for ( int secondNumber = firstNumber ; secondNumber > 1 ; secondNumber-- )  {
				// 「空白」を表示する。ループによって繰り返し表示する。
				System.out.print(" ");
			}
			// 「*」を表示するループ。繰り返す回数を入力した値の回数から縦軸の段数が変わるごとに1回ずつ減らしていく。
